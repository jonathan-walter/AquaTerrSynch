...
### This version starts with chla and adds additional info about color, DOC, nitrate + nitrite,
### total N, total P, and Secchi depth
...
vars<-vars[!is.na(vars$chla),] #remove obs with missing data
...
for(id in unique(vars$lagoslakeid)){
  ...
  #id lakes with chla data spanning >= 20 years, 
  ...
  #id lakes with >= 20 consecutive years of chla data
...
d20obs<-vars[vars$lagoslakeid %in% check.20obs,]
d20consec<-vars[vars$lagoslakeid %in% check.20consec,]

pdf("~/Box Sync/NSF EAGER Synchrony/Data Availability Studies/map_20chla_obs.pdf")
plot(states, main="lakes with >=20 chla observations", xlim=lim.x, ylim=lim.y)
points(d20obs$nhd_long,d20obs$nhd_lat, pch=".", cex=3, col="red")
dev.off()
...
for(id in check.20obs){
  ...
  tmpout<-c(id,
            ...
            sum(is.na(vars.sub$colora))/nrow(vars.sub),
            sum(is.na(vars.sub$colort))/nrow(vars.sub),
            sum(is.na(vars.sub$doc))/nrow(vars.sub),
            sum(is.na(vars.sub$no2no3))/nrow(vars.sub),
            sum(is.na(vars.sub$tn))/nrow(vars.sub),
            sum(is.na(vars.sub$tp))/nrow(vars.sub),
            sum(is.na(vars.sub$secchi))/nrow(vars.sub)
            )
  summdf.20obs<-rbind(summdf.20obs,tmpout)
}
colnames(summdf.20obs)<-c("lagoslakeid","no.yrs.chla","avg.obs_yr.chla","sd.obs_yr.chla","min.month.chla","max.month.chla",
...
pdf("~/Box Sync/NSF EAGER Synchrony/Data Availability Studies/map_20chla_obs_allyr.pdf")
...
colnames(summdf.20consec)<-c("lagoslakeid","no.yrs.chla","avg.obs_yr.chla","sd.obs_yr.chla","min.month.chla","max.month.chla",
                             "prNA.colora","prNA.colort","prNA.doc","prNA.no2no3","prNA.tn","prNA.tp","prNA.secchi")
...
pdf("~/Box Sync/NSF Eager Synchrony/Data Availability Studies/map_chla_20consecyr.pdf")
