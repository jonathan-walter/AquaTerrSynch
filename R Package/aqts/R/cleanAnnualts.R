...
#' and completeness requirements set by \code{ymin} and \code{maxNA}. Leading and trailing NAs are trimmed from the time series. Missing values
#' are filled only if they are non-consecutive. If no variables meet requirements, the whole lake is dropped from the output. If some, but not 
#' all variables meet requirements, data are returned with the offending variables ommitted. Data should still be inspected and explored prior to 
