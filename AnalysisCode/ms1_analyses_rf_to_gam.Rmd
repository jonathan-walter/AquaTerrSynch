...
load("~/Box Sync/NSF EAGER Synchrony/Data/RData files/ms1_analysis_inprogress1_v10873.RData")
...
dt.geo<-dt$lakes.geo
dt.geo<-dt$lakes.geo[,!colnames(dt.geo) %in% c("state_zoneid","iws_zoneid","edu_zoneid")]
...
print(varimp.coh.st[order(varimp.coh.st, decreasing=T)][1:20])
#hist(predcoh.st)
...
gam.cohst<-gam(accndvicoh.ts1 ~ s(buffer500m_streamdensity_midreaches_density_mperha) + s(hu12_nlcd2011_pct_31 ) +
               s(buffer500m_streamdensity_streams_density_mperha) + s(hu12_baseflowindex_mean) + 
                 s(hu12_prism_ppt_30yr_normal_800mm2_annual_mean), data=rfdat.cohst, gamma=1, weights=lwgt)
gam.check(gam.cohst)
...
rfdat.cohlt<-rfdat.cohlt[,!grepl("borderhu12s",rfdat.cohlt)]
...
gam.cohlt<-gam(accndvicoh.ts2 ~ s(hu12_nlcd2011_pct_90 ) + s(hu12_canopy2001_mean) + s(cv.accndvi) + 
                 s(hu12_dep_totaln_tavg_mean) + hu6_zoneid, data=rfdat.cohlt, gamma=1, weights=lwgt)
gam.check(gam.cohlt)
...
print(varimp.phi.st[order(varimp.phi.st, decreasing=T)][1:20])
...
gam.phist<-gam(cos(accndviphi.ts1) ~ s(maxdepth) + s(upstream_lakes_4ha_count) + 
                 s(wlconnections_openwaterwetlands_contributing_area_), 
...
print(varimp.phi.lt[order(varimp.phi.lt, decreasing=T)][1:20])
