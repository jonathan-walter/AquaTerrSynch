...
varimp.phi.st<-varimp(cf.cosphist)
...
varimp.phi.st<-varimp(cf.sinphist)
print(varimp.phi.st[order(varimp.phi.st, decreasing=T)][1:10])
...
gam.sinphist<-gam(sin(accndviphi.ts1) ~ s(buffer500m_streamdensity_headwaters_density_mperha) + 
                 ...
                 s(buffer500m_streamdensity_streams_density_mperha), 
               data=rfdat.phist, gamma=1, weights=lwgt)
gam.check(gam.sinphist)
concurvity(gam.sinphist)
...
cor.test(pred.cosphi.lt,cos(rfdat.cosphilt$accndviphi.ts2))
...
#cosine
gam.cosphilt<-gam(cos(accndviphi.ts2) ~ hu4_zoneid + s(hu12_tri_mean) + 
                 s(hu12_nlcd2011_pct_41), 
               data=rfdat.philt, gamma=1, weights=lwgt)
gam.check(gam.cosphilt)
concurvity(gam.cosphilt)
summary(gam.cosphilt)
plot(cos(rfdat.cosphilt$accndviphi.ts2), predict(gam.cosphilt, rfdat.philt))
