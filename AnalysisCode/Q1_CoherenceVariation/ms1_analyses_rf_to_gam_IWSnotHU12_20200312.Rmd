...
gam.cohst<-gam(accndvicoh.ts1 ~ s(hu12_dep_totaln_tavg_mean) + s(log10(cv.accndvi)) + s(iws_nlcd2011_pct_90,k=3)
               #+ s(wlconnections_allwetlands_shoreline_km) + s(hu12_runoff_mean)
               , data=rfdat.cohst, gamma=1, weights=lwgt, family=betar)
gam.check(gam.cohst)
...
gam.cohlt<-gam(accndvicoh.ts2 ~ s(iws_nlcd2011_pct_52) + s(log10(cv.accndvi)) + 
               ...
               , data=rfdat.cohlt, gamma=1, weights=lwgt, family="betar")
gam.check(gam.cohlt)
...
#cosine
gam.cosphist<-gam(cos(accndviphi.ts1) ~ s(sqrt(wlconnections_openwaterwetlands_shoreline_km),k=3) + 
                  s(log10(chla)) + s(iws_nlcd2011_pct_52,k=3), data=rfdat.phist, gamma=1, weights=lwgt)
...
#sine
gam.sinphilt<-gam(sin(accndviphi.ts2) ~ s(sqrt(wlconnections_openwaterwetlands_shoreline_km)) + 
                 s(iws_nlcd2011_pct_82) + s(sqrt(iws_wl_regimefwetlandsundissolved_overlapping_area_pct)), 
               data=rfdat.philt, gamma=1, weights=lwgt)
...
tiff("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5_gamfits_bycolumn_iws_v2.tif",units="in",res=300,width=bigwd, height=bight)
...
plot(gam.cohst,select=1,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="total N deposition",ylab="short coherence", ylim=c(-2,3.2))
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"a)")
plot(gam.cohst,select=2,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab=expression(paste("log"[10],"(cv(NDVI))",sep="")),ylab="short coherence", ylim=c(-2,3.2))
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"b)")
plot(gam.cohst,select=3,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="% woody wetlands",ylab="",ylim=c(-2,3.2))
...
plot(gam.cosphist,select=1,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="sqrt(open wetld. shoreline)",ylab="short phase")
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"d)")
plot(gam.cosphist,select=2,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab=expression(paste("log"[10],"(mean chlorophyll-a)",sep="")),ylab="")
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"e)")
plot(gam.cosphist,select=3,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="% shrub/scrub",ylab="")
...
plot(gam.cohlt,select=1,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="% shrub/scrub",ylab="long coherence",ylim=c(-2.5,2.2))
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"g)")
plot(gam.cohlt,select=2,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab=expression(paste("log"[10],"(cv(NDVI))",sep="")),ylab="long coherence",ylim=c(-2.5,2.2))
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"h)")
plot(gam.cohlt,select=3,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="% regime f wetlands",ylab="",ylim=c(-2.5,2.2))
...
plot(gam.sinphilt,select=1,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="sqrt(open wetld. shoreline)",ylab="")
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"j)")
plot(gam.sinphilt,select=2,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="% cultivated crops",ylab="")
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"k)")
plot(gam.sinphilt,select=3,residuals=T,rug=FALSE,shade=T,pch=20,cex=.2,xlab="sqrt(% regime f wetlands)",ylab="")
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"l)")
