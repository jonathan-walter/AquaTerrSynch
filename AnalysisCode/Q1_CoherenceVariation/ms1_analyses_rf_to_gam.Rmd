...
bgcol[names(lakedata)=="4386"]<-"blue"
...
points(analysislakes$lakeinfo$nhd_long, analysislakes$lakeinfo$nhd_lat, pch=21, cex=0.55, col="black",bg=bgcol)
legend("bottomright",pch=c(0,21,21,21),col="black",pt.bg=c("grey","grey","red","blue"),
...
## try out portrait orientation by column
...
png("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5b_hu8_cohst_bycol.png", units="in",
     res=300, width=bigwd/4, height=bight/5)
par(mar=c(0,0,1,0))
plot(huc_bdys_cohst,bty="o",xlab="HU8 watershed", col=pal[scale1_100(huc_bdys_cohst$coeff)],xlim=c(-91837,2260000),
     ylim=c(1454000,3012981),lwd=0.5)
lines(lagosstates_prj,lwd=0.5)
...
png("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5i_hu8_cohlt_bycol.png", units="in",
     res=300, width=bigwd/4, height=bight/5)
par(mar=c(0,0,1,0))
plot(huc_bdys_cohlt,bty="o",xlab="HU8 watershed", col=pal[scale1_100(huc_bdys_cohlt$coeff)],xlim=c(-91837,2260000),
     ylim=c(1454000,3012981),lwd=0.5)
...
png("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5n_hu4_sinphilt_bycol.png", units="in",
     res=300, width=bigwd/4, height=bight/5)
par(mar=c(0,0,1,0))
plot(huc_bdys_sinphilt,bty="o",xlab="HU4 watershed", col=pal[scale1_100(huc_bdys_sinphilt$coeff)],xlim=c(-91837,2260000),
     ylim=c(1454000,3012981),lwd=0.5)
...
panel5b<-readPNG("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5b_hu8_cohst_bycol.png")
panel5i<-readPNG("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5i_hu8_cohlt_bycol.png")
panel5n<-readPNG("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5n_hu4_sinphilt_bycol.png")

mar1<-c(3,1.5,0.3,0.2)
fudge=1/60

tiff("~/Box Sync/NSF EAGER Synchrony/Manuscripts/1_CoherenceSpatialVariation/fig5_gamfits_bycolumn.tif",units="in",res=300,width=bigwd, height=bight)

par(mfcol=c(5,4),mgp=c(1.5,0.5,0), oma=c(0,1.5,1.5,0.5),mar=mar1,tcl=-0.3)
...
plot(gam.cohlt,select=4,residuals=T,rug=FALSE,shade=T,cex=2,xlab="all wetld. contrib. area",ylab="",ylim=c(-0.5,1.5))
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"m)")

plot(NA,NA,xlim=c(0,1),ylim=c(0,1),xaxs="i",yaxs="i",ylab="",xlab="HUC-4 sub-region",xaxt="n",yaxt="n",mgp=c(1,0.5,0))
rasterImage(image=panel5n,xleft=1e-2,ybottom=1e-2,xright=1-1e-2,ytop=1-1e-2)
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"n)")
plot(gam.sinphilt,select=1,residuals=T,rug=FALSE,shade=T,cex=2,xlab="% pasture",ylab="")
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"o)")
plot(gam.sinphilt,select=2,residuals=T,rug=FALSE,shade=T,cex=2,xlab="% devel.-open space",ylab="")
text(par("usr")[1]+0.075*diff(par("usr")[1:2]), par("usr")[4]-0.075*diff(par("usr")[3:4]),"p)")

mtext("Partial residuals",2,outer=T,line=0.1,cex=1)
